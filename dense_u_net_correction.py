...
class DensenetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False,n_blocks=9, padding_type='reflect', nb_layers=7, growth_rate=16):
        ...
        for i in range(num_updown_block):
            enc_list.append(DenseBlock(nb_layers=nb_layers, in_planes=ngf * mult, growth_rate=growth_rate, block=BottleneckBlock,use_dropout=use_dropout))
            enc_list.append(TransitionBlock(ngf * mult + nb_layers*growth_rate, ngf * mult, use_dropout=use_dropout))
        ...
        for i in range(num_updown_block):
            dec_list.append(
                nn.Sequential(
                DenseBlock(nb_layers=nb_layers, in_planes=2 * ngf * mult + nb_layers*growth_rate, growth_rate=growth_rate, block=BottleneckBlock,use_dropout=use_dropout),
                TransitionBlock(2 * ngf * mult + 2*nb_layers*growth_rate, ngf * mult, use_dropout=use_dropout)
                )
                )
